...
<head>
  ...
  <script type="text/javascript">
	function deletecoffee(num, currentPage){
		if(confirm("선택하신 제품을 삭제하시겠습니까?")){
			location.href ="deletemenu.do?num="+num+"&pagekind=coffee&page="+currentPage;
		}		
	}
  </script>
</head>
...
<!--   <br> -->
  ...
  <ul class="nav nav-pills nav-justified nav-danger hidden-xs">
    ...
    <li class="pills-active"><a href="#" id="colorwhite">커피</a></li>
...
<form action="<%=request.getContextPath()%>/menu/coffee.js">
  ...
  <img class="img-responsive" src='<%=request.getContextPath() + "/resources/upload/menu"%>/${coffeelist.filename}'
  alt="${coffeelist.filename} 이미지" style="width: 200px; height: 200px;"></a>
  <p class="text-center">${coffeelist.hotncold} ${coffeelist.menuname}</p>
  </div>
  <!-- Modal -->
    ...
    <!-- Modal content-->
      <div class="modal-content">
      	...
      	<div class="modal-body">
      		...
      		<img class="img-responsive" src='<%=request.getContextPath() + "/resources/upload/menu"%>/${coffeelist.filename}'
  alt="${coffeelist.filename} 이미지">
      		</div>
      		<div class="col-sm-8">
      		<h3 style="padding-bottom:5px; border-bottom: 1px dotted #990000;">${coffeelist.menuname}</h3>
			<p>${coffeelist.description}</p>
			<table class="table">
			<tr><th>구분</th><th>열량(kcal)</th><th>당류(g)</th><th>단백질(g)</th><th>카페인(mg)</th><th>포화지방(g)</th><th>나트륨(mg)</th></tr>
			<tr><td>${coffeelist.hotncold}</td><td>${coffeelist.calory}</td><td>${coffeelist.saccharide}</td>
			<td>${coffeelist.protein}</td><td>${coffeelist.caffeine}</td><td>${coffeelist.fat}</td><td>${coffeelist.sodium}</td>
			</tr>
			</table>
			</div>
			</div>
    	...
    	<div class="modal-footer">
    	  <c:if test="${sessionScope.grade == 'ADMIN' }">
    	  	<button type="button" class="btn btn-warning" data-dismiss="modal"
    	  		onclick="location.href='<%=request.getContextPath()%>/menu/updatemenu.js?num=${coffeelist.num}&pagekind=coffee&page=${currentPage}'">상품수정</button>
    	  	<button type="button" class="btn btn-warning" data-dismiss="modal"
    	  		onclick="deletecoffee(${coffeelist.num},${currentPage})">상품삭제</button>
