...
<head>
  ...
  <script type="text/javascript">
	function deletedessert(num, currentPage){
		if(confirm("선택하신 제품을 삭제하시겠습니까?")){
			location.href ="deletemenu.do?num="+num+"&pagekind=dessert&page="+currentPage;
		}		
	}
  </script>
</head>
...
<!--   <h1 class="text-center">MENU</h1>
  ...
  <ul class="nav nav-pills nav-justified nav-danger hidden-xs">
    ...
    <li class="pills-active"><a href="#" id="colorwhite">디져트</a></li>
  ...
  <h2>디져트</h2>
  &nbsp;총 디져트 종류: ${realTotalNum}
  </div>
<form action="<%=request.getContextPath()%>/menu/dessert.js">
  ...
  <div class="row">
  <%int i=0; %>
  <c:forEach var="dessertlist" items="${dessertlist}">
  <%i=i+1; %>
  <!-- Trigger the modal with a button -->
  <div class="col-lg-3 col-md-6 thumb thumb_bottom">
  <a class="thumbnail" href="#" data-toggle="modal" data-target="#<%=i%>">
  <img class="img-responsive" src='<%=request.getContextPath() + "/resources/upload/menu"%>/${dessertlist.filename}'
  alt="${dessertlist.filename} 이미지" style="width: 200px; height: 200px;"></a>
  <p class="text-center">${dessertlist.menuname}</p>
  </div>
  <!-- Modal -->
    ...
    <!-- Modal content-->
      <div class="modal-content">
      	...
      	<div class="modal-body">
      		...
      		<img class="img-responsive" src='<%=request.getContextPath() + "/resources/upload/menu"%>/${dessertlist.filename}'
  alt="${dessertlist.filename} 이미지">
      		</div>
      		<div class="col-sm-8">
      		<h3 style="padding-bottom:5px; border-bottom: 1px dotted #990000;">${dessertlist.menuname}</h3>
			<p>${dessertlist.description}</p>
			<table class="table">
			<tr><th>구분</th><th>열량(kcal)</th><th>당류(g)</th><th>단백질(g)</th><th>카페인(mg)</th><th>포화지방(g)</th><th>나트륨(mg)</th></tr>
			<tr><td>${dessertlist.hotncold}</td><td>${dessertlist.calory}</td><td>${dessertlist.saccharide}</td>
			<td>${dessertlist.protein}</td><td>${dessertlist.caffeine}</td><td>${dessertlist.fat}</td><td>${dessertlist.sodium}</td>
			</tr>
			</table>
			</div>
			</div>
    	...
    	<div class="modal-footer">
    	  <c:if test="${sessionScope.grade == 'ADMIN' }">
    	  	<button type="button" class="btn btn-warning" data-dismiss="modal"
    	  		onclick="location.href='<%=request.getContextPath()%>/menu/updatemenu.js?num=${dessertlist.num}&pagekind=dessert&page=${currentPage}'">상품수정</button>
    	  	<button type="button" class="btn btn-warning" data-dismiss="modal"
    	  		onclick="deletedessert(${dessertlist.num},${currentPage})">상품삭제</button>
  ...
  <c:if test="${sessionScope.grade == 'ADMIN' }">  	
  <button type="button" class="btn btn-danger col-md-offset-10 col-md-1"
  	onclick="location.href='<%=request.getContextPath()%>/menu/addmenu.js?pagekind=dessert&page=${currentPage}'"
  	style="float: right;">상품추가</button>
  </c:if>
  </div>  
</div>
