...
<head>
  ...
  <script type="text/javascript">
	function deletebeverage(num, currentPage){
		if(confirm("선택하신 제품을 삭제하시겠습니까?")){
			location.href ="deletemenu.do?num="+num+"&pagekind=beverage&page="+currentPage;
		}		
	}
  </script>
</head>
...
<!--   <h1 class="text-center">MENU</h1>
  ...
  <ul class="nav nav-pills nav-justified nav-danger hidden-xs">
    ...
    <li class="pills-active"><a href="#" id="colorwhite">음료</a></li>
  ...
  <h2>음료</h2>
  &nbsp;총 음료 종류: ${realTotalNum}
  </div>
<form action="<%=request.getContextPath()%>/menu/beverage.js" method="post" enctype="multipart/form-data">
  ...
  <div class="row">
  <%int i=0; %>
  <c:forEach var="beveragelist" items="${beveragelist}">
  <!-- Trigger the modal with a button -->
  <div class="col-lg-3 col-md-6 thumb thumb_bottom">
  <%i=i+1; %>
  <a class="thumbnail" href="#" data-toggle="modal" data-target="#<%=i%>">
  <img class="img-responsive" src='<%=request.getContextPath() + "/resources/upload/menu"%>/${beveragelist.filename}'
  alt="${beveragelist.filename} 이미지" style="width: 200px; height: 200px;"></a>
  <p class="text-center">${beveragelist.menuname}</p>
  </div>
  <!-- Modal -->
    ...
    <!-- Modal content-->
      <div class="modal-content">
      	...
      	<div class="modal-body">
      		...
      		<img class="img-responsive" src='<%=request.getContextPath() + "/resources/upload/menu"%>/${beveragelist.filename}'
  alt="${beveragelist.filename} 이미지">
      		</div>
      		<div class="col-sm-8">
      		<h3 style="padding-bottom:5px; border-bottom: 1px dotted #990000;">${beveragelist.menuname}</h3>
			<p>${beveragelist.description}</p>
			<table class="table">
			<tr><th>구분</th><th>열량(kcal)</th><th>당류(g)</th><th>단백질(g)</th><th>카페인(mg)</th><th>포화지방(g)</th><th>나트륨(mg)</th></tr>
			<tr><td>${beveragelist.hotncold}</td><td>${beveragelist.calory}</td><td>${beveragelist.saccharide}</td>
			<td>${beveragelist.protein}</td><td>${beveragelist.caffeine}</td><td>${beveragelist.fat}</td><td>${beveragelist.sodium}</td>
			</tr>
			</table>
			</div>
			</div>
    	...
    	<div class="modal-footer">
    	  <c:if test="${sessionScope.grade == 'ADMIN' }">
    	  	<button type="button" class="btn btn-warning" data-dismiss="modal"
    	  		onclick="location.href='<%=request.getContextPath()%>/menu/updatemenu.js?num=${beveragelist.num}&pagekind=beverage&page=${currentPage}'">상품수정</button>
    	  	<button type="button" class="btn btn-warning" data-dismiss="modal"
    	  		onclick="deletebeverage(${beveragelist.num},${currentPage})">상품삭제</button>
  ...
  <c:if test="${sessionScope.grade == 'ADMIN' }">  	
  <button type="button" class="btn btn-danger col-md-offset-10 col-md-1"
  	onclick="location.href='<%=request.getContextPath()%>/menu/addmenu.js?pagekind=beverage&page=${currentPage}'"
  	style="float: right;">상품추가</button>
  </c:if>
  </div>  
</div>
