...
<html lang="ko">
<head>
  <title>(Admin)CafeJo - 프렌차이즈 문의 답변</title>
  <meta charset="utf-8">
...
<c:if test="${sessionScope.grade=='ADMIN' }">
  <h2 class="text-center">프렌차이즈 창업 문의 답글</h2>
  <form class="form-horizontal" action="<%= request.getContextPath() %>/franchise/addadminqna.do" method="post" enctype="multipart/form-data">
  <input id="qref" name="qref" type="hidden" value="${param.qref}">
  <table class="table">
  ...
  <tr>
  		<td colspan="2">
  		<button type="submit" class="btn btn-danger col-md-offset-5 col-xs-2">답변하기</button>
  		</td>
  </tr>
  </table>
  </form>
...
<c:if test="${ sessionScope.grade !='ADMIN' }">
<h1 class="text-center"><span class="glyphicon glyphicon-ban-circle"></span> 관리자 전용 페이지 입니다. </h1>
<br>
