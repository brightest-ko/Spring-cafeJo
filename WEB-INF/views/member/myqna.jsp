...
<div class="row">	
  ...
  <tr style="background-color: #b03838; color: white;" class="hidden-xs">
    <th>제목</th><th>작성일</th><th>문의 현황</th>
  </tr>
  ...
  <tr>
    <td onclick="location.href='<%=request.getContextPath()%>/franchise/qnaview.js?qnum=${myqnalist.qnum}&page=${currentPage}'"
    id="titlelink" class="hidden-xs">
  	<c:if test="${myqnalist.qreply == 'thisreply'}">    
    &nbsp;&nbsp;└&nbsp;&nbsp;${myqnalist.qtitle}
	</c:if>
  	<c:if test="${myqnalist.qreply != 'thisreply'}">
    ${myqnalist.qtitle}
    </c:if></td>
    <td class="hidden-xs">${myqnalist.qnadate}</td>
    
    <td onclick="location.href='<%=request.getContextPath()%>/franchise/qnaview.js?qnum=${myqnalist.qnum}&page=${currentPage}'"
    id="titlelink" class="hidden-sm hidden-md hidden-lg">
  	<c:if test="${myqnalist.qreply == 'thisreply'}">    
    <div id="title1">&nbsp;&nbsp;└&nbsp;&nbsp;${myqnalist.qtitle}</div>
	</c:if>
  	<c:if test="${myqnalist.qreply != 'thisreply'}">
    <div id="title1">${myqnalist.qtitle}</div>
    </c:if>
    <div style="color: gray; font: 5px;">${myqnalist.qnadate}</div></td>
  	
  	<c:if test="${myqnalist.qreply == 'noreply'}">    
    <td style="color: #b03838;"> 접수 중 </td>
    </c:if>
  	<c:if test="${myqnalist.qreply == 'yesreply'}">    
